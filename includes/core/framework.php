...
// Don't duplicate me!
if( !class_exists( 'IMIC_Framework' ) ) {

    /**
     * Main IMIC_Framework class
     *
     * @since       1.0.0
     */
    class IMIC_Framework {
...
/**
        Still need to port these.
            ...
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . __( 'Options panel created using', 'imic-framework') . ' <a href="' . $this->framework_url . '" target="_blank">' . __('IMIC Framework', 'imic-framework') . '</a> v' . self::$_version . '</span>';
            $defaults['help_tabs']          = array();
            $defaults['help_sidebar']       = ''; // __( '', 'imic-framework' );
...
**/
            ...
            self::$_properties = array( 
                'args' => array(
                    'opt_name' => array(
                            ...
                            'data_type'=>'string', 
                            'label'=>'Option Name', 
                            'desc'=>'Must be defined by theme/plugin. Is the unique key allowing multiple instance of IMIC within a single Wordpress instance.', 
                            'default'=>''
                        ),
                    ...
                    'menu_title' => array( 
                            ...
                            'default'=> __( 'Options', 'imic-framework' )
                        ),              
                    'page_title' => array( 
                            'data_type'=>'string', 
                            'label'=>'Page Title', 
        ...
        protected $framework_url        = 'http://www.imicreation.com/';
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
			 ...
			 */
            $defaults['opt_name'] = isset($args['opt_name']) && $args['opt_name'] != '' ? $args['opt_name'] : 'imic_demo';
            ...
            $defaults['menu_title']         = __( 'Options', 'imic-framework' );
            $defaults['page_icon']          = 'icon-themes';
            $defaults['page_title']         = __( 'Options', 'imic-framework' );
	    ...
	    if ( empty( $this->path ) ) {
		    ...
		    $this->sections = apply_filters('imic-sections',$sections); // REMOVE LATER
            $this->sections = apply_filters('imic/options/'.$this->args['opt_name'].'/sections',$this->sections);
            ...
            add_action( 'do_feed_imicopts-' . $this->args['opt_name'], array( &$this, '_download_options' ), 1, 1 );
		...
		public function _internationalization( $opt_name ) {
            ...
            $domain = 'imic-framework';
            $locale = apply_filters( 'imic/textdomain/'. $opt_name, get_locale(), $domain );
        ...
        public function _tracking() {
			...
			/** @noinspection PhpUnusedLocalVariableInspection */
			$imic_tracking = new IMIC_Tracking($this);
        }
		...
		function set_options( $value = '' ) {
			...
			if( !empty($value) ) {
				...
				if ( $this->args['global_variable'] ) {
					...
					global $$options;
                    $value = apply_filters( 'imic/options/'.$this->args['opt_name'].'/global_variable', $value );
					$$options = $value;					
				}
				do_action( 'imic-saved-' . $this->args['opt_name'] , $value ); // REMOVE
                do_action( 'imic/options/'.$this->args['opt_name'].'/saved', $value );
			}
		}
		...
		function get_options() {
			...
			if ( $this->args['global_variable'] ) {
				$options = $this->args['global_variable'];
				global $$options;
                $result = apply_filters( 'imic/options/'.$this->args['opt_name'].'/global_variable', $result );
				$$options = $result;			
			}
			//print_r($result);
			return $result;
...
/**
		...
		function get_wordpress_data($type = false, $args = array()) {
			...
			if ( !empty($type) ) {
                ...
                $data = apply_filters( 'imic/options/'.$this->args['opt_name'].'/wordpress_data/'.$type.'/', $data ); // REMOVE LATER
                $data = apply_filters( 'imic/options/'.$this->args['opt_name'].'/data/'.$type, $data ); 
				...
				**/
				if (!empty($type) && empty($data)) {
					...
					if ($type == "categories" || $type == "category") {
						...
						$font_icons = apply_filters('imic-font-icons',array()); // REMOVE LATER
                        $font_icons = apply_filters('imic/font-icons',$font_icons);
        ...
        public function _default_values() {
            ...
            $this->options_defaults = apply_filters( 'imic/options/'.$this->args['opt_name'].'/defaults', $this->options_defaults );
...
//                $comparison = $field['required'][1];
        ...
        public function _set_default_options() {
		    ...
		    // Get args
		    $this->args = apply_filters( 'imic-args-'.$this->args['opt_name'], $this->args ); // REMOVE
            $this->args = apply_filters( 'imic/options/'.$this->args['opt_name'].'/args', $this->args );
		    ...
		    // Get sections
		    $this->sections = apply_filters( 'imic-sections-' . $this->args['opt_name'], $this->sections ); // REMOVE
            $this->sections = apply_filters( 'imic/options/' . $this->args['opt_name'].'/sections', $this->sections );
            ...
            if( $this->args['page_type'] == 'submenu' ) {
                ...
                if( true === $this->args['allow_sub_menu'] ) {
                    ...
                    if( true === $this->args['show_import_export'] ) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __( 'Import / Export', 'imic-framework' ),
                            __( 'Import / Export', 'imic-framework' ),
                            $this->args['page_cap'],
                ...
                if( isset( $section['fields'] ) ) {
					...
					foreach( $section['fields'] as $fieldk => $field ) {
						if( isset( $field['type'] ) && $field['type'] != "callback"  ) {
                            $field_class = 'IMIC_Framework_' . $field['type'];
                            if( !class_exists( $field_class ) ) {
//                                $class_file = apply_filters( 'imic/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                                $class_file = apply_filters( 'imic/'.$this->args['opt_name'].'/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field );
        ...
        public function _enqueue() {
            ...
            $wp_styles->add_data( 'imic-elusive-icon-ie7', 'conditional', 'lte IE 7' );

            wp_register_style(
                'jquery-ui-css',
                apply_filters( 'imic/page/'.$this->args['opt_name'].'/enqueue/jquery-ui-css', self::$_url . 'assets/css/vendor/jquery-ui-bootstrap/jquery-ui-1.10.0.custom.css' ),
                '',
                time(),
                'all'
            );
            ...
            wp_enqueue_style( 'imic-lte-ie8' );

            wp_enqueue_style( 'imic-css' );

            wp_enqueue_style( 'imic-elusive-icon' );
            wp_enqueue_style( 'imic-elusive-icon-ie7' );
            ...
            $localize = array(
                    'save_pending'      => __( 'You have changes that are not saved. Would you like to save them now?', 'imic-framework' ), 
                    'reset_confirm'     => __( 'Are you sure? Resetting will loose all custom values.', 'imic-framework' ), 
                    'preset_confirm'    => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'imic-framework' ), 
            ...
            $errors = get_transient( 'imic-errors-' . $this->args['opt_name'] );
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && !empty( $errors ) ) {
            	...
            	delete_transient( 'imic-errors-' . $this->args['opt_name'] );
            	$localize['errors'] = array('total'=>$theTotal, 'errors'=>$theErrors);
            }

            // Construct the errors array. 
            $warnings = get_transient( 'imic-warnings-' . $this->args['opt_name'] );
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && !empty( $warnings ) ) {
            	...
            	delete_transient( 'imic-warnings-' . $this->args['opt_name'] );
            ...
            do_action( 'imic-enqueue-' . $this->args['opt_name'], $this ); // REMOVE
            do_action( 'imic/page/' . $this->args['opt_name'] . '/enqueue' );
...
//                                $class_file = apply_filters( 'imic/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
        ...
        public function _download_options(){
            ...
            $backup_options = $this->get_options( str_replace( 'imic-', '', $_GET['feed'] ) );
            $backup_options['imic-backup'] = '1';
            $content = json_encode( $backup_options );

            if( isset( $_GET['action'] ) && $_GET['action'] == 'download_options' ) {
                header( 'Content-Description: File Transfer' );
                header( 'Content-type: application/txt' );
                header( 'Content-Disposition: attachment; filename="' . str_replace( 'imic-', '', $_GET['feed'] ) . '_backup_' . date( 'd-m-Y' ) . '.json"' );
        ...
        public function _load_page() {
            ...
            do_action( 'imic-load-page-' . $this->args['opt_name'], $screen ); // REMOVE
            do_action( 'imic/page/' . $this->args['opt_name'] . '/load' , $screen );
        }

        /**
         * Do action imic-admin-head for options page
         *
         * @since       1.0.0
         * @access      public
         * @return      void
        ...
        public function admin_head() {
            do_action( 'imic-admin-head-' . $this->args['opt_name'], $this ); // REMOVE
            do_action( 'imic/page/' . $this->args['opt_name'] . '/header', $this );
        }
        ...
        public function _register_setting() {
            ...
            foreach( $this->sections as $k => $section ) {
                ...
                $section = apply_filters( 'imic-section-' . $k . '-modifier-' . $this->args['opt_name'], $section );
                $section = apply_filters( 'imic/options/'.$this->args['opt_name'].'/section/' . $section['id'] , $section );
                    ...
                    foreach( $section['fields'] as $fieldk => $field ) {
						...
						if ( $this->args['default_show'] === true && isset( $field['default'] ) && isset($this->options[$field['id']]) && $this->options[$field['id']] != $field['default'] && $field['type'] !== "info" && $field['type'] !== "group" ) {
							...
							if ( !empty( $default_output ) ) {
							    $default_output = __( 'Default', 'imic-framework' ) . ": " . substr($default_output, 0, -2);
							}				   	
			            ...
			            $field = apply_filters( 'imic-field-' . $field['id'] . 'modifier-' . $this->args['opt_name'], $field ); // REMOVE LATER
                        $field = apply_filters( 'imic/options/' . $this->args['opt_name'].'/field/' . $field['id'], $field );
                        ...
                        do_action( 'imic/options/'.$this->args['opt_name'].'/field/'.$field['type'].'/register', $field);
            ...
            do_action( 'imic-register-settings-' . $this->args['opt_name'] ); // REMOVE
            do_action( 'imic/options/'.$this->args['opt_name'].'/register', $this->sections);

			if ($runUpdate) { // Always update the DB with new fields
				$this->set_options( $this->options );
			}

			if (get_transient( 'imic-compiler-' . $this->args['opt_name'] ) ) {
				delete_transient( 'imic-compiler-' . $this->args['opt_name'] );
				do_action( 'imic-compiler-' . $this->args['opt_name'], $this->options ); // REMOVE
                do_action( 'imic/options/' . $this->args['opt_name'] . '/compiler', $this->options );
			}				
        ...
        public function _register_extensions() {        	
		    ...
		    foreach($folders as $folder){
				...
				$extension_class = 'IMIC_Framework_Extension_' . $folder;

                if( !class_exists( $extension_class ) ) {
                    $class_file = apply_filters( 'imic-extensionclass-load', $this->path . 'extensions/' . $folder . '/extension_' . $folder . '.php', $extension_class ); // REMOVE LATER
                    $class_file = apply_filters( 'imic/extension/'.$this->args['opt_name'].'/'.$folder, $this->path . 'extensions/' . $folder . '/extension_' . $folder . '.php', $class_file );
		    ...
		    do_action( 'imic-register-extensions-' . $this->args['opt_name'], $this ); // REMOVE
            do_action( 'imic/extensions/'.$this->args['opt_name'], $this );       			
        ...
        public function _validate_options( $plugin_options ) {
            ...
            set_transient( 'imic-saved-' . $this->args['opt_name'], '1', 1000 );
            ...
            if( !empty( $plugin_options['import'] ) ) {
                ...
                if( !empty( $imported_options ) && is_array( $imported_options ) && isset( $imported_options['imic-backup'] ) && $imported_options['imic-backup'] == '1' ) {
                    $plugin_options['IMIC_imported'] = 1;
                	foreach($imported_options as $key => $value) {
                		$plugin_options[$key] = $value;
                	}                    
                    
                    // Remove the import/export tab cookie.
                    if( $_COOKIE['imic_current_tab'] == 'import_export_default' ) {
                        setcookie( 'imic_current_tab', '', 1, '/' );
                    }

                    set_transient( 'imic-compiler-' . $this->args['opt_name'], '1', 1000 );
                    $plugin_options['IMIC_COMPILER'] = time();
            	...
            	$plugin_options['IMIC_imported'] = false;
            }
            ...
            if( $this->errors ) {
            	set_transient( 'imic-errors-' . $this->args['opt_name'], $this->errors, 1000 );
            }

            if( $this->warnings ) {
            	set_transient( 'imic-warnings-' . $this->args['opt_name'], $this->warnings, 1000 );
            }               

            do_action_ref_array('imic-validate-' . $this->args['opt_name'], array(&$plugin_options, $this->options)); // REMOVE
            do_action_ref_array('imic/options/' . $this->args['opt_name'].'/validate', array(&$plugin_options, $this->options));

            if( !empty( $plugin_options['compiler'] ) ) {
            	$plugin_options['IMIC_COMPILER'] = time();
            	set_transient( 'imic-compiler-' . $this->args['opt_name'], '1', 2000 );
            }
                ...
                if( isset( $section['fields'] ) ) {
                    foreach( $section['fields'] as $field ) {
                        ...
                        if( isset( $field['validate'] ) ) {
                            $validate = 'IMIC_Validation_' . $field['validate'];

                            if( !class_exists( $validate ) ) {
                                $class_file = apply_filters( 'imic-validateclass-load', self::$_dir . 'inc/validation/' . $field['validate'] . '/validation_' . $field['validate'] . '.php', $validate ); // REMOVE LATER
                                $class_file = apply_filters( 'imic/validate/'.$this->args['opt_name'].'/class/'.$field['validate'], self::$_dir . 'inc/validation/' . $field['validate'] . '/validation_' . $field['validate'] . '.php', $class_file );
        ...
        public function _options_page_html() {
            ...
            $saved = get_transient( 'imic-saved-' . $this->args['opt_name'] );
            if ( $saved ) {
            	delete_transient( 'imic-saved-' . $this->args['opt_name'] );	
            }
            ...
            // Do we support JS?
            echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'imic-framework' ) . '</div></noscript>';

            // Security is vital!
            echo '<input type="hidden" id="ajaxsecurity" name="security" value="' . wp_create_nonce( 'of_ajax_nonce' ) . '" />';

            do_action( 'imic-page-before-form-' . $this->args['opt_name'] ); // Remove
            do_action( 'imic/page/'.$this->args['opt_name'].'/form/before', $this );

            // Main container
            echo '<div id="imic-container">';
            echo '<form method="post" action="' . './options.php" enctype="multipart/form-data" id="imic-form-wrapper">';

            echo '<input type="hidden" id="imic-compiler-hook" name="' . $this->args['opt_name'] . '[compiler]" value="" />';
            ...
            if( isset( $this->args['intro_text'] ) ) {
                echo '<div id="imic-intro-text">';
                echo $this->args['intro_text'];
                echo '</div>';
            }

            // Stickybar
            echo '<div id="imic-sticky">';
            echo '<div id="info_bar">';
            echo '<a href="javascript:void(0);" id="expand_options">' . __( 'Expand', 'imic-framework' ) . '</a>';
            echo '<div class="imic-action_bar">';
            submit_button( __( 'Save Changes', 'imic-framework'), 'primary', 'imic_save', false );
            echo '&nbsp;';
            submit_button( __( 'Reset to Defaults', 'imic-framework' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';

            echo '<div class="imic-ajax-loading" alt="' . __( 'Working...', 'imic-framework' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';

            // Warning bar
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
                if( isset( $this->options['IMIC_imported'] ) && $this->options['IMIC_imported'] === 1 ) {
                    echo '<div id="imic-imported">' . apply_filters( 'imic-imported-text-' . $this->args['opt_name'], '<strong>' . __( 'Settings Imported!', 'imic-framework' ) ) . '</strong></div>';
                } else {
                    echo '<div id="imic-save">' . apply_filters( 'imic-saved-text-' . $this->args['opt_name'], '<strong>'.__( 'Settings Saved!', 'imic-framework' ) ) . '</strong></div>';
                }
            }

            echo '<div id="imic-save-warn">' . apply_filters( 'imic-changed-text-' . $this->args['opt_name'], '<strong>'.__( 'Settings have changed, you should save them!', 'imic-framework' ) ) . '</strong></div>';
            echo '<div id="imic-field-errors"><strong><span></span> ' . __( 'error(s) were found!', 'imic-framework' ) . '</strong></div>';
            echo '<div id="imic-field-warnings"><strong><span></span> ' . __( 'warning(s) were found!', 'imic-framework' ) . '</strong></div>';
            ...
            // Sidebar
            echo '<div id="imic-sidebar">';
            echo '<ul id="imic-group-menu">';
            foreach( $this->sections as $k => $section ) {
				...
				if (isset($section['type']) && $section['type'] == "divide") {
	                ...
	                echo '<li id="' . $k . '_section_group_li" class="imic-group-tab-link-li">';
	                echo '<a href="javascript:void(0);" id="' . $k . '_section_group_li_a" class="imic-group-tab-link-a" data-rel="' . $k . '">' . $icon . '<span class="group_title">' . $section['title'] . '</span></a>';
	                if ( !empty( $section['sections'] ) ) {
	                	echo '<ul id="' . $k . '_section_group_li_subsections" class="sub">';
	                	foreach ($section['sections'] as $k2 => $subsection) {
	                		echo '<li id="' . $k . '_section_group_li" class="imic-group-tab-link-li">';
	                		echo '<a href="javascript:void(0);" id="' . $k . '_section_group_subsection_li_a" class="imic-group-tab-link-a" data-rel="' . $k .'sub-'.$k2.'"><span class="group_title">' . $subsection['title'] . '</span></a>';
	                		echo '</li>';
	                	}
	                	echo '</ul>';
	                }
            ...
            do_action( 'imic-page-after-sections-menu-' . $this->args['opt_name'], $this );
            do_action( 'imic/page/'.$this->args['opt_name'].'/menu/after', $this );

            if( $this->args['show_import_export'] === true ) {
                echo '<li id="import_export_default_section_group_li" class="imic-group-tab-link-li">';
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="imic-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . __( 'Import / Export', 'imic-framework' ) . '</span></a>';
                echo '</li>';
            ...
            if( $this->args['dev_mode'] === true ) {
                echo '<li id="dev_mode_default_section_group_li" class="imic-group-tab-link-li">';
            ...
            if( $this->args['system_info'] === true ) {
                echo '<li id="system_info_default_section_group_li" class="imic-group-tab-link-li">';
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="imic-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'imic-framework' ) . '</span></a>';
                echo '</li>';
            }
            ...
            echo '<div id="imic-main">';

            foreach( $this->sections as $k => $section ) {
                echo '<div id="' . $k . '_section_group' . '" class="imic-group-tab">';
                if ( !empty( $section['sections'] ) ) {
                	//$tabs = "";
		            echo '<div id="' . $k . '_section_tabs' . '" class="imic-section-tabs">';
		            echo '<ul>';                	
		            ...
		            echo '</ul>';
               		foreach ($section['sections'] as $subkey => $subsection) {
               			echo '<div id="' . $k .'sub-'.$subkey. '_section_group' . '" class="imic-group-tab">';
            ...
            if( $this->args['show_import_export'] === true ) {
                echo '<div id="import_export_default_section_group' . '" class="imic-group-tab">';

                echo '<h3>' . __( 'Import / Export Options', 'imic-framework' ) . '</h3>';
				echo '<h4>' . __( 'Import Options', 'imic-framework' ) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="imic-import-code-button" class="button-secondary">' . __( 'Import from file', 'imic-framework' ) . '</a> <a href="javascript:void(0);" id="imic-import-link-button" class="button-secondary">' . __( 'Import from URL', 'imic-framework' ) . '</a></p>';

                echo '<div id="imic-import-code-wrapper">';

                echo '<div class="imic-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters( 'imic-import-file-description', __( 'Input your backup file below and hit Import to restore your sites options from a backup.', 'imic-framework' ) ) . '</p>';
                echo '</div>';
                ...
                echo '<div id="imic-import-link-wrapper">';

                echo '<div class="imic-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters( 'imic-import-link-description', __( 'Input the URL to another sites options set and hit Import to load the options from that site.', 'imic-framework' ) ) . '</p>';
                echo '</div>';
                ...
                echo '<p id="imic-import-action"><input type="submit" id="imic-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __( 'Import', 'imic-framework' ) . '">&nbsp;&nbsp;<span>' . apply_filters( 'imic-import-warning', __( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'imic-framework' ) ) . '</span></p>';
                echo '<div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>';

                echo '<h4>' . __( 'Export Options', 'imic-framework' ) . '</h4>';
                echo '<div class="imic-section-desc">';
                echo '<p class="description">' . apply_filters( 'imic-backup-description', __( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'imic-framework' ) ) . '</p>';
                echo '</div>';

                /** @noinspection PhpUndefinedConstantInspection */
                echo '<p><a href="javascript:void(0);" id="imic-export-code-copy" class="button-secondary">' . __( 'Copy', 'imic-framework' ) . '</a> <a href="' . add_query_arg( array( 'feed' => 'imicopts-' . $this->args['opt_name'], 'action' => 'download_options', 'secret' => md5( AUTH_KEY . SECURE_AUTH_KEY ) ), site_url() ) . '" id="imic-export-code-dl" class="button-primary">' . __( 'Download', 'imic-framework' ) . '</a> <a href="javascript:void(0);" id="imic-export-link" class="button-secondary">' . __( 'Copy Link', 'imic-framework' ) . '</a></p>';
                $backup_options = $this->options;
                $backup_options['imic-backup'] = '1';
                echo '<textarea class="large-text noUpdate" id="imic-export-code" rows="8">';
                print_r( json_encode( $backup_options ) );
                echo '</textarea>';
                /** @noinspection PhpUndefinedConstantInspection */
                echo '<input type="text" class="large-text noUpdate" id="imic-export-link-value" value="' . add_query_arg( array( 'feed' => 'imicopts-' . $this->args['opt_name'], 'secret' => md5( AUTH_KEY.SECURE_AUTH_KEY ) ), site_url() ) . '" />';

                echo '</div>';
            }

            if( $this->args['dev_mode'] === true ) {
                echo '<div id="dev_mode_default_section_group' . '" class="imic-group-tab">';
                echo '<h3>' . __( 'Options Object', 'imic-framework' ) . '</h3>';
                echo '<div class="imic-section-desc">';

                echo '<div id="imic-object-browser"></div>';

                echo '</div>';

                echo '<div id="imic-object-json" class="hide">'.json_encode($this->options).'</div>';

                echo '<a href="#" id="consolePrintObject" class="button">' . __( 'Show Object in Javascript Console Object', 'imic-framework' ) . '</a>';
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<div id="system_info_default_section_group' . '" class="imic-group-tab">';
                echo '<h3>' . __( 'System Info', 'imic-framework' ) . '</h3>';

                echo '<div id="imic-system-info">';
            ...
            do_action( 'imic/page-after-sections-' . $this->args['opt_name'], $this ); // REMOVE LATER
            do_action( 'imic/page/'.$this->args['opt_name'].'/sections/after', $this );

            echo '<div class="clear"></div>';
            echo '</div>';
            echo '<div class="clear"></div>';

            echo '<div id="imic-sticky-padder" style="display: none;">&nbsp;</div>';
            echo '<div id="imic-footer-sticky"><div id="imic-footer">';

            if( isset( $this->args['share_icons'] ) ) {
                echo '<div id="imic-share">';
            ...
            echo ( isset( $this->args['footer_text'] ) ) ? '<div id="imic-sub-footer">' . $this->args['footer_text'] . '</div>' : '';

            do_action( 'imic-page-after-form-' . $this->args['opt_name'] ); // REMOVE
            do_action( 'imic/page/'.$this->args['opt_name'].'/form/after', $this );
            ...
            if ( $this->args['dev_mode'] === true ) {
            	...
            	echo '<br /><div class="imic-timer">' . get_num_queries() . ' queries in ' . timer_stop(0) . ' seconds</div>';
        ...
        public function _section_desc( $section ) {
            ...
            if( isset( $this->sections[$id]['desc'] ) && !empty( $this->sections[$id]['desc'] ) ) {
            	echo '<div class="imic-section-desc">' . $this->sections[$id]['desc'] . '</div>';
            }
        }
        ...
        public function _field_input( $field, $v = "" ) {
            ...
            if( isset( $field['callback'] ) && function_exists( $field['callback'] ) ) {
                $value = ( isset( $this->options[$field['id']] ) ) ? $this->options[$field['id']] : '';
                do_action( 'imic-before-field-' . $this->args['opt_name'], $field, $value ); // REMOVE
                do_action( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/callback/before', $field, $value );
                do_action( 'imic/field/'.$this->args['opt_name'].'/callback/before', $field, $value );
                call_user_func( $field['callback'], $field, $value );
                do_action( 'imic-after-field-' . $this->args['opt_name'], $field, $value ); // REMOVE
                do_action( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/callback/after', $field, $value );
                do_action( 'imic/field/'.$this->args['opt_name'].'/callback/after', $field, $value );
                return;
            }
...
//                    $class_file = apply_filters( 'imic/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                ...
                if( class_exists( $field_class ) ) {
                    ...
                    do_action( 'imic-before-field-' . $this->args['opt_name'], $field, $value ); // REMOVE
                    do_action( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/render/before', $field, $value );
                    do_action( 'imic/field/'.$this->args['opt_name'].'/render/before', $field, $value );

                    $render = new $field_class( $field, $value, $this );
                    ob_start();
					/** @noinspection PhpUndefinedMethodInspection */
					$render->render();
                    $_render = apply_filters( 'imic-field-'.$this->args['opt_name'], ob_get_contents(), $field ); // REMOVE
                    $_render = apply_filters( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/render/after', $_render, $field );
                    $_render = apply_filters( 'imic/field/'.$this->args['opt_name'].'/render/after', $_render, $field );
                    ob_end_clean();
                    ...
                    do_action( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/fieldset/before/' . $this->args['opt_name'], $field, $value );
                    do_action( 'imic/field/'.$this->args['opt_name'].'/fieldset/before/' . $this->args['opt_name'], $field, $value );
					echo '<fieldset id="'.$this->args['opt_name'].'-'.$field['id'].'" class="imic-field imic-container-'.$field['type'].' '.$class_string.'" data-id="'.$field['id'].'" '.$data_string.'>';
	                    echo $_render;
                    ...
                    do_action( 'imic-after-field-' . $this->args['opt_name'], $field, $value ); // REMOVE
                    do_action( 'imic/field/'.$this->args['opt_name'].'/'.$field['type'].'/fieldset/after/' . $this->args['opt_name'], $field, $value );
                    do_action( 'imic/field/'.$this->args['opt_name'].'/fieldset/after/' . $this->args['opt_name'], $field, $value );
                }
            }
        } // function
