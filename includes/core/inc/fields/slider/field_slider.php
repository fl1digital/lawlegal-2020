<?php
class IMIC_Framework_slider extends IMIC_Framework{	
	...
	*/
	function render(){
		...
		global $wp_scripts;
		$data = $wp_scripts->get_data('imic-field-slider-js', 'data');

		if(!empty($data)) { // Adding to the previous localize script object
		  if(!is_array($data)) {
		    $data = json_decode(str_replace('var imicSliders = ', '', substr($data, 0, -1)), true);
		  }
		  foreach($data as $key => $value) {
		    $localized_data[$key] = $value;
		  }
		  $wp_scripts->add_data('imic-field-slider-js', 'data', '');
		}
		$localized_data[$this->field['id']] = $params;
		wp_localize_script('imic-field-slider-js', 'imicSliders', $localized_data);		
		...
		echo '<div id="'.$this->field['id'].'-slider" class="imic_slider" rel="'.$this->field['id'].'"></div>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			'imic-typewatch-js', 
			IMIC_Framework::$_url.'assets/js/vendor/jquery.typewatch.min.js', 
			array('jquery'),
			time(),
			true
		);			
		...
		wp_enqueue_script(
			'imic-field-slider-js', 
			IMIC_Framework::$_url.'inc/fields/slider/field_slider.min.js', 
			array('jquery', 'jquery-numeric', 'jquery-ui-core', 'jquery-ui-slider', 'jquery-ui-dialog', 'imic-typewatch-js'),
			time(),
			true
		);		

		wp_enqueue_style(
			'imic-field-slider-css', 
			IMIC_Framework::$_url.'inc/fields/slider/field_slider.css', 
			time(),
			true
		);		
