...
function imic_add_file(event, selector) {
	...
	frame.on( 'select', function() {
		...
		frame.close();
		if (typeof imicMedia[jQuery(selector).attr('data-id')] !== 'undefined' && attachment.attributes.type !== imicMedia[jQuery(selector).attr('data-id')]) {
			return;
		}
		...
		imic_change( jQuery(selector).find( '.upload-id' ) );
		...
		if ( !selector.find('.upload').hasClass('noPreview') ) {
			selector.find('.screenshot').empty().hide().append('<img class="imic-option-image" src="' + thumbSrc + '">').slideDown('fast');
		}
		//selector.find('.media_upload_button').unbind();
		selector.find('.remove-image').removeClass('hide');//show "Remove" button
		selector.find('.imic-background-properties').slideDown();
	});
...
function imic_remove_file(selector) {
	...
	imic_change( jQuery(selector).find( '.upload-id' ) );
	selector.find('.imic-background-properties').hide();
	...
	*/
    $.imic.media = function(){
		...
		$('.remove-image, .remove-file').unbind('click').on('click', function() {
			imic_remove_file( $(this).parents('fieldset.imic-field:first') );
		});

		// Upload media button
		$('.media_upload_button').unbind().on('click', function( event ) {
			imic_add_file( event, $(this).parents('fieldset.imic-field:first') );
		});
    };
