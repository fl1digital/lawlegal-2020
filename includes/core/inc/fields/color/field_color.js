/*global jQuery, document, imic_change */
(function($){
	'use strict';
	...
	$.imic.color = function(){
		...
		$('.imic-color').on('focus', function() {
			$(this).data('oldcolor', $(this).val());
		});

		$('.imic-color').on('keyup', function() {
			var value = $(this).val();
			var color = imic_color_validate(this);
			var id = '#' + $(this).attr('id');
		...
		$('.imic-color').on('blur', function() {
			...
			if (value === "transparent") {
				...
				if (imic_color_validate(this) === value) {
					if (value.indexOf("#") !== 0) {
		...
		$('.imic-color').on('keydown', function() {
...
// Run the validation
function imic_color_validate(field) {
