/* global confirm, imic_opts */

jQuery(document).ready(function() {

	// On label click, change the input and class
	jQuery('.imic-image-select label img, .imic-image-select label .tiles').click(function(e) {
		var id = jQuery(this).closest('label').attr('for');
		jQuery(this).parents("fieldset:first").find('.imic-image-select-selected').removeClass('imic-image-select-selected');
		jQuery(this).closest('label').find('input[type="radio"]').prop('checked');
		if (jQuery(this).closest('label').hasClass('imic-image-select-presets')) { // If they clicked on a preset, import!
			e.preventDefault();
			var presets = jQuery(this).closest('label').find('input');
			var data = presets.data('presets');
			if (presets !== undefined && presets !== null) {
				var answer = confirm(imic_opts.preset_confirm);
				if (answer) {
					jQuery('label[for="' + id + '"]').addClass('imic-image-select-selected');
					window.onbeforeunload = null;
					jQuery('#import-code-value').val(JSON.stringify(data));
					jQuery('#imic-import').click();
				}
			} else {}
			return false;
		} else {
            imic_change(jQuery(this).closest('label').find('input[type="radio"]'));
			jQuery('label[for="' + id + '"]').addClass('imic-image-select-selected');
		}
	});
