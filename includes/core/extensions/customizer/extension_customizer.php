...
// Don't duplicate me!
if( !class_exists( 'IMIC_Framework_extension_customizer' ) ) {
     ...
     * @since       1.0.0
     */
    class IMIC_Framework_extension_customizer extends IMIC_Framework {
...
//echo "there";
  //      print_r($wp_customize);
      ...
      public function _enqueue_previewer() {
        wp_enqueue_script( 'imic-extension-previewer-js', $this->extension_url . 'assets/js/preview.js' );
        $localize = array(
          'save_pending'      => __( 'You have changes that are not saved. Would you like to save them now?', 'imic-framework' ), 
          'reset_confirm'     => __( 'Are you sure? Resetting will lose all custom values.', 'imic-framework' ), 
        ...
        wp_localize_script( 'imic-extension-previewer-js', 'imiccPost', $localize);
      } 
      ...
      public function _enqueue() {
        ...
        wp_enqueue_script('imic-extension-customizer-js', $this->extension_url . 'assets/js/customizer.js');
        // Get styles
        wp_enqueue_style('imic-extension-customizer-css', $this->extension_url . 'assets/css/customizer.css');
        ...
        do_action( 'imic-enqueue-' . $this->args['opt_name'] );
            ...
            foreach( $section['fields'] as $field ) {
              if( isset( $field['type'] ) ) {
                ...
                if( !class_exists( $field_class ) ) {
                  $class_file = apply_filters( 'imic-typeclass-load', $this->path . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field_class );
                  if( $class_file ) {
