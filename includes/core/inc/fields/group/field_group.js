...
(function($){
    ...
    $.imic.group = function(){
        ...
        $('.imic-groups-accordion-group input[type=text]').on('keyup',function(event) {
            $(this).parents('.imic-groups-accordion-group:first').find('.imic-groups-header').text(event.target.value);
            $(this).parents('.imic-groups-accordion-group:first').find('.slide-title').val(event.target.value);
        });
        
        $('.imic-groups-remove').live('click', function () {
            imic_change($(this));
        ...
        $('.imic-groups-add').click(function () {

            var newSlide = $(this).prev().find('.imic-groups-accordion-group:last').clone(true);
            var slideCount = $(newSlide).find('input[type="text"]').attr("name").match(/\d+(?!.*\d+)/);
            var slideCount1 = slideCount*1 + 1;

            $(newSlide).find('h3').text('').append('<span class="imic-groups-header">New Group</span><span class="ui-accordion-header-icon ui-icon ui-icon-plus"></span>');
